...
ClusterData process_files(const string& sim_file, const string& p_file, 
    ...
    // Load neighbor data
    vector<vector<int>> neighbor_lists(N);
    vector<int> neighbor_counts(N);
    ...
    string line;
    while (getline(neighbor_stream, line)) {
...
int main(int argc, char* argv[]) {
    ...
    for (int sim_idx = 1; sim_idx <= nsims; sim_idx++) {
        for (int t_val = 0; t_val <= t_max; t_val += t_step) {
