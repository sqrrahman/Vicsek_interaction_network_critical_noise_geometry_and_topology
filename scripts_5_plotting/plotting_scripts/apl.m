...
for i = 2
    ...
    for j = groups{i}
        ...
        y = data(:, 5); % Average Path length
        std = data(:, 6); % Population standard deviation
        ...
        % if j == 4
        %     draw_slope(x(2:end),y(2:end),1,length(y)-1,0.85);
        % end
    end
    plot_styling();
end

filename = sprintf('apl.png');
...
function plot_styling()
    ...
    ylabel('$\langle \ell \rangle$', 'Interpreter', 'latex', 'FontSize', 16);
