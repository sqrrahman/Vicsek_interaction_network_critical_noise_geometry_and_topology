...
for j = [4,5,6]
    ...
    plot(x, y, 'Color', colors{j-3}, 'LineStyle', '-', 'LineWidth', 1.5);
    fill(xx, yy, colors{j-3}, 'FaceAlpha', 0.2, 'EdgeColor', 'none', 'HandleVisibility','off');

    % if j == 4
    %     draw_slope(x,y,7,21,1.6);
    % end
end
plot_styling_a()


filename = sprintf('cmpd_v.png');
print(gcf, filename, '-dpng', '-r600');



function plot_styling_a()
    ...
    legend_labels = {'$s = 0.1$' '$s = 0.25$' '$s = 1.0$'};
    legend(legend_labels, 'Interpreter','latex','FontSize',16,'Location','sw');

    % text(0.98, 0.98, '(a)', 'Units','normalized','FontSize',16,'Interpreter','latex','HorizontalAlignment','right','VerticalAlignment','top');
end
